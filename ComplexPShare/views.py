...
from ComplexPShare import app,db
...
def profile(user_id):
     ...
     if user == None:
         return redirect('/')
     paginate = Image.query.filter_by(user_id=user_id).paginate(page=1,per_page=6,error_out=False)
     return render_template('profile.html',has_next=paginate.has_next,user=user,images=paginate.items)
